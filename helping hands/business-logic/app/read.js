...
read.logic = async (req) => {
    ...
    const [count, lists] = await Promise.all([
        database.client.collection(collections.LISTS).countDocuments(key),
        database.client.collection(collections.LISTS).find(key).skip(offset).limit(limit).toArray()
    ]);

    const collection = await Promise.all(lists.map(async item => {
        const {uid} = item;
